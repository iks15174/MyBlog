...
function update_backend_container() {
    ...
    docker compose up -d --no-deps $BACKEND1_SERVICE_NM # 첫번째 backend service를 다시 시작한다.
    ...
    docker compose up -d --no-deps  $BACKEND2_SERVICE_NM # 두번째 backend service를 다시 시작한다.
}